...
@call "%~dp0buildtools\get_build_deps.cmd" win.durango
@if %ERRORLEVEL% neq 0 (
    @exit /b %errorlevel%
)

:: Run packman to ensure dependencies are present and run cmake generation script afterwards
@echo Getting target platform dependencies for xboxone ...
@call "%~dp0buildtools\packman5\packman.cmd" pull "%~dp0target_platform_deps_xboxone.xml" --platform xboxone --postscript "%~dp0buildtools\cmake_projects_xboxone.bat"
@if %ERRORLEVEL% neq 0 (
    @exit /b %errorlevel%
) else (
    @echo Success!
