...
This application can load model file sets that are generated by the :ref:`pageauthoring` tool.  A valid file set for this sample is one of the following:

# An ExtPxAsset file and a graphics file:
  * .blast file: contains the Nv::Blast::ExtPxAsset which stores collision hulls in addition to Blast data.  Here you must use the --px option in when exporting from :ref:`pageauthoring`.
  * .obj or .fbx file: stores the graphics data.
# An NvBlastAsset or TkAsset file, and a graphics + collision file:
  * .blast file: contains an NvBlastAsset or Nv::Blast::TkAsset (use the --ll or --tk option, respectively, when exporting from :ref:`pageauthoring`).
  * .fbx file: stores the graphics data along with embedded collision data.  Here you must use the --fbxcollision option when exporting from :ref:`pageauthoring` to embed collision information.

.. _assetviewer_file_load:
...
Running the application with no commandline arguments, the viewer will load **samples/resources/configs/assets.xml** by default.  This file references
the models in the subdirectories of **samples/resources/models**.
...
You may also press **SPACE** to toggle between damage and drag mode.  (The mode is shown at the top of the screen.)  In drag mode, dynamic objects can be manipulated by
placing the mouse cursor over them, and using **Left Mouse / Drag** to pull on the object.  In damage mode, **left-clicking** a destructible actor will apply damage to it.
...
# White - falloff damage.  Damage is applied everywhere within the sphere.
# Blue - cutter damage.  Damage is applied only near the surface of the sphere.
# Green - hierarchical damage.  Damage is passed down the chunk hierarchy with every damage application, sometimes leading to smaller chunks from an initial damage application.
...
Replay control section allows to control recording and replaying Blast events. It demonstrates the usage of :ref:`ExtSync` extension. You may start/stop recording of Blast events (damage, split). 
If you toggle to sync initial actors, once recording starts, the full Blast state is saved.
...
This menu controls PhysXsimulation parameters.  Here you may choose to use fixed time steps (off by default), and set the time step (if Use Fixed Timestep is checked) with the Fixed Timestep control.
