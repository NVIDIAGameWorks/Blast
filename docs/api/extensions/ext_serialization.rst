.. _pageextserialization:

Serialization (NvBlastExtSerialization)
---------------------------------------

.. _serialization_intro:
...
with types enumerated in the header **NvBlastExtLlSerialization.h**.
...
so it requires the last parameter to tell it what object it is serializing.  A utility wrapper function is given in **NvBlastExtLlSerialization.h** which performs the
