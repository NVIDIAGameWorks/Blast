...
@call "%~dp0buildtools\get_build_deps.cmd" win.orbis
@if %ERRORLEVEL% neq 0 exit /b %errorlevel%

@echo Getting target platform dependencies for ps4 ...
@call "%~dp0buildtools\packman5\packman.cmd" pull "%~dp0target_platform_deps_ps4.xml" --postscript "%~dp0buildtools\cmake_projects_ps4.bat"
@if %ERRORLEVEL% neq 0 (
    @exit /b %errorlevel%
) else (
    @echo Success!
